...
**Goal:** 创建 game_automate.py 实现游戏自动化辅助工具，支持两种检测模式，打包成 exe 运行。

**Architecture:** 基于现有 V1/yolo_automate.py 进行重构，提取可复用组件，新增模式二逻辑和固定坐标配置。采用单文件设计便于打包。

**Tech Stack:** Python 3.x, OpenCV DNN, PyInstaller, win32gui, pynput
...
```
D:\Projects\yolo-opencv-detector-main\
├── game_automate.py          # 新建：主程序（单文件包含所有组件）
├── auto-run.json             # 新建：配置文件
├── yolov4-tiny/
│   ├── obj.names             # 已有：类别名称
│   ├── yolov4-tiny-custom.cfg # 已有：模型配置
│   └── yolov4-tiny-custom_last.weights # 已有：权重文件（需确认存在）
└── docs/superpowers/specs/   # 设计文档（已完成）
```

---

### Task 1: 创建配置管理器 ConfigManager

**Files:**
- Create: `game_automate.py` (新增 ConfigManager 类)

- [ ] **Step 1: 编写 ConfigManager 类定义和默认配置**

在 `game_automate.py` 开头添加导入和 ConfigManager 类：
...
运行: `python -c "import ast; ast.parse(open('game_automate.py').read())"`
预期: 无输出表示语法正确

- [ ] **Step 3: 提交**

```bash
git add game_automate.py
git commit -m "feat: add ConfigManager class for auto-run.json"
```

---

### Task 2: 创建窗口选择器 WindowSelector

**Files:**
- Modify: `game_automate.py` (追加 WindowSelector 类)

- [ ] **Step 1: 编写 WindowSelector 类**

追加到 `game_automate.py`：
...
```bash
git add game_automate.py
git commit -m "feat: add WindowSelector class for window listing"
```

---

### Task 3: 创建窗口截图类 WindowCapture

**Files:**
- Modify: `game_automate.py` (追加 WindowCapture 类)

- [ ] **Step 1: 编写 WindowCapture 类**
...
```bash
git add game_automate.py
git commit -m "feat: add WindowCapture class for screenshot"
```

---

### Task 4: 创建图像处理器 ImageProcessor

**Files:**
- Modify: `game_automate.py` (追加 ImageProcessor 类)

- [ ] **Step 1: 编写 ImageProcessor 类**
...
```bash
git add game_automate.py
git commit -m "feat: add ImageProcessor class with YOLO detection"
```

---

### Task 5: 创建自动化引擎 AutomationEngine

**Files:**
- Modify: `game_automate.py` (追加 AutomationEngine 类)

- [ ] **Step 1: 编写 AutomationEngine 类**
...
```bash
git add game_automate.py
git commit -m "feat: add AutomationEngine with mode1 and mode2 logic"
```
...
**Files:**
- Modify: `game_automate.py` (追加 draw_detection 方法)

- [ ] **Step 1: 编写可视化绘制函数**

追加到 `game_automate.py` (在 AutomationEngine 类之后添加独立函数):
...
```bash
git add game_automate.py
git commit -m "feat: add draw_detection function for visualization"
```
...
**Files:**
- Modify: `game_automate.py` (追加 get_user_config 函数)
...
```bash
git add game_automate.py
git commit -m "feat: add get_user_config function for interactive setup"
```
...
**Files:**
- Modify: `game_automate.py` (追加 main 函数和入口)
...
```bash
git add game_automate.py
git commit -m "feat: add main function and program entry point"
```
...
**Files:**
- Create: `build_game_automate.bat` (打包脚本)

- [ ] **Step 1: 编写打包脚本**

创建 `build_game_automate.bat`：
...
```bash
git add build_game_automate.bat
git commit -m "feat: add PyInstaller build script for exe packaging"
```
...
```bash
git add -A
git commit -m "feat: complete game automation tool with mode1 and mode2"
```
...
**1. Spec 覆盖检查:**
- ✅ Task 1-3: ConfigManager + WindowSelector + WindowCapture = 需求 2, 3
- ✅ Task 5: AutomationEngine mode1_process/mode2_process = 需求 4
- ✅ Task 6: draw_detection 显示玩家坐标 = 需求 5
- ✅ Task 9: 打包脚本 = 需求 1
